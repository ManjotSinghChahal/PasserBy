...
import kotlinx.android.synthetic.main.fragment_passerby_adv.view.*


class PasserbyAdv : Fragment() {


    val v:String?=null
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
       val view = inflater.inflate(R.layout.fragment_passerby_adv, container, false)

        view.next_passerbyAdv.setOnClickListener {
            activity?.let {
                //   it. supportFragmentManager.beginTransaction().replace(R.id.container_home, UserList()).commit()
                it. supportFragmentManager.beginTransaction().replace(R.id.container_home, MapAdv()).commit()
            }
        }

        view.passerby_skip.setOnClickListener {
