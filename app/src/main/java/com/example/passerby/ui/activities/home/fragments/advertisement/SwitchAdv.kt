...
import kotlinx.android.synthetic.main.fragment_switch_adv.view.*


class SwitchAdv : Fragment() {

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
      val view = inflater.inflate(R.layout.fragment_switch_adv, container, false)


        view.next_switchAdv.setOnClickListener {
            activity?.let {
                //   it. supportFragmentManager.beginTransaction().replace(R.id.container_home, UserList()).commit()
                it. supportFragmentManager.beginTransaction().replace(R.id.container_home, ReadyToGo()).commit()
            }
        }

        view.switchAdv_skip.setOnClickListener {
